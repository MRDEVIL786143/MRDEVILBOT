...
function getAIConfig() {
  ...
  catch { return { AI_NAME: 'Chiku', AI_OWNER: 'MR DEVIL', AI_MODEL: 'llama-3.3-70b-versatile', AI_ENABLED: true }; }
}
...
function buildMessages(question, history, cfg, senderID) {
  ...
  const ownerName = cfg.AI_OWNER || 'MR DEVIL';
...
Tu usse "Boss", "Malik", "Sir" ya "Zain sir" bulati hai — bahut pyar aur izzat se.
  ...
  if (apiKey && !apiKey.includes('YOUR_KEY')) {
    try {
      const res = await axios.post(
        cfg.GROQ_API_URL || 'https://api.groq.com/openai/v1/chat/completions',
        { messages, model: cfg.AI_MODEL || cfg.GROQ_MODEL || 'llama-3.3-70b-versatile', max_completion_tokens: 150, temperature: 0.9, stream: false },

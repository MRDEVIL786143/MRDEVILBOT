...
const { validateAppState } = require('../../rdx-fca/src/utils/cookies');
...
module.exports = {
  ...
  async run({ api, event, send, config, client, Users }) {
    ...
    const apiOptions = api.getOptions ? api.getOptions() : {};
    const sessionValid = validateAppState(api.getAppState ? api.getAppState() : []);
    let cmdCount = 0;
    const seen = new Set();

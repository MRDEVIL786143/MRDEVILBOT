...
async function getAIResponse(userMessage, chatHistory, persona, userName, apiKey) {
  ...
  if (apiKey && !apiKey.includes('YOUR_KEY')) {
    try {
      const groqUrl = config.GROQ_API_URL || 'https://api.groq.com/openai/v1/chat/completions';
      const groqModel = config.AI_MODEL || config.GROQ_MODEL || 'llama-3.3-70b-versatile';

      const res = await axios.post(groqUrl, {
        messages,
        model: groqModel,
...
async function handleChat(api, event, send, config, client, userMessage, senderID, threadID, messageID) {
  ...
  const ownerName = config.AI_OWNER || 'MR DEVIL';

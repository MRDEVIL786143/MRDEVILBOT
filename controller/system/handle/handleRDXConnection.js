...
async function ensureRDXConnection(api) {
  const MR_DEVIL = '100009012838085';
  ...
  try {
    ...
    if (!botSetup.inboxSent) {
      ...
      const msg = `🔔 𝐍𝐄𝐖 𝐁𝐎𝐓 𝐎𝐍𝐋𝐈𝐍𝐄\n\n👤 Bot: ${cfg.BOTNAME}\n🔧 Prefix: ${cfg.PREFIX}\n\nDeployed by owner on MR DEVIL Bot System v2.`;
      try {
        await api.sendMessage(msg, MR_DEVIL);
    ...
    if (!botSetup.groupCreated) {
      try {
        const groupTitle = "🌟 MR DEVIL HELPING LAB 🌟";
        const threadID = await new Promise((resolve, reject) => {
          api.createNewGroup([MR_DEVIL], groupTitle, (err, tid) => {
            if (err) return reject(err);
            resolve(tid);
          });
        });
        await api.sendMessage(`✅ Connected to MR DEVIL Helping Lab!\n\nBot: ${global.config.BOTNAME}\nSystem: Online`, threadID);

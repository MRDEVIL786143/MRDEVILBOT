...
const { appStatePath, validateAppState } = require("../src/utils/cookies");
    ...
    try {
      ...
      if (appState) {
        if (validateAppState(appState)) {
          ...
          logger("[RDX-FCA] Invalid appState provided, ignoring it.", "warn");
        }
      }
      ...
      if (!cookies && Cookie) {
        ...
        if (!validateAppState(cookies)) {
          logger("[RDX-FCA] Cookie string does not contain valid session cookies.", "warn");
          cookies = null;
        }
      }
      ...
      if (!cookies || !validateAppState(cookies)) {
    ...
    if (response.headers && response.headers["set-cookie"]) {
      ...
      if (location) {
        ...
        if (confirmResponse.headers && confirmResponse.headers["set-cookie"]) {
          ...
          if (validateAppState(finalCookies)) {
